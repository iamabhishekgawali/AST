...
#    python universal_analyzer_final.py ./PythonAST ./python.config.json
...
def finalize_report(stats: Dict[str, Any], lang_config: Dict[str, Any]) -> Dict[str, Any]:
    """Assembles the final report with the user-specified structure."""
    ...
    # Safely get the list of environment variables
    env_vars_stats = stats['patterns'].get('Environment Variables Accessed', {'list': set()})
    detected_env_vars = list(env_vars_stats['list'])
    
    # --- Assemble the new report structure ---
    report = {
        ...
        "EnviornmentvariablesDetected": detected_env_vars
    }
    return report
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python universal_analyzer_final.py <path-to-ast-directory> <path-to-config.json>")
    else:
